...
export default function GameSetup() {
  ...
  // Starting Lineups
  const [lineupA, setLineupA] = useState(Array(6).fill(null)); // P1-P6 positions
  ...
  const handleCoinTossChange = (field, value) => {
    const newCoinToss = { ...coinToss, [field]: value };
    
    // Auto-assign teams if coin toss is complete
    if (field === 'winner' && value && newCoinToss.choice) {
      if (value === 'team1') {
        if (newCoinToss.choice === 'serve' || newCoinToss.choice === 'receive') {
  ...
  const getTeamAssignment = () => {
    ...
    const teamALineup = coinToss.teamAAssignment === 'team1' ? lineupA : lineupB;
    const teamBLineup = coinToss.teamBAssignment === 'team1' ? lineupA : lineupB;
    ...
    try {
      ...
      const gameData = {
        ...
        teams: {
          A: {
            players: playersA,
            lineup: assignment.teamALineup.filter(p => p !== null)
          },
          B: {
            players: playersB,
            lineup: assignment.teamBLineup.filter(p => p !== null)
          }
        },
        sets: [{
          setNumber: 1,
      ...
      <div className="setup-wrapper">
        <div className="setup-header">
          ...
          <div className="setup-progress">
            ...
            <div className={`progress-step ${currentStep >= 2 ? 'active' : ''}`}>2. Officials</div>
            <div className={`progress-step ${currentStep >= 3 ? 'active' : ''}`}>3. Teams</div>
            <div className={`progress-step ${currentStep >= 4 ? 'active' : ''}`}>4. Coin Toss</div>
            <div className={`progress-step ${currentStep >= 5 ? 'active' : ''}`}>5. Rosters</div>
        ...
        {currentStep === 4 && (
          <div className="setup-step">
            ...
            <div className="setup-buttons">
              ...
              <button onClick={() => setCurrentStep(5)} disabled={!coinToss.teamAAssignment || !coinToss.teamBAssignment}>Next →</button>
            </div>
          </div>
        )}
              ...
              <div className="roster-team">
                ...
                <div className="roster-table-wrapper">
                  <table className="roster-table">
                    ...
                    <tbody>
                      {assignment.teamARoster.map((player, i) => (
                        <tr key={i}>
                          <td>{i + 1}</td>
                          <td>
                            <input
                              ...
                              value={player.jersey}
                              onChange={(e) => {
                                const rosterNum = coinToss.teamAAssignment === 'team1' ? 1 : 2;
                                updateRoster(rosterNum, i, 'jersey', e.target.value);
                              }}
                              placeholder="#"
                            />
                          </td>
                          <td>
                            <input
                              ...
                              onChange={(e) => {
                                const rosterNum = coinToss.teamAAssignment === 'team1' ? 1 : 2;
                                updateRoster(rosterNum, i, 'name', e.target.value);
                              }}
                          ...
                          <td>
                            <select
                              ...
                              onChange={(e) => {
                                const rosterNum = coinToss.teamAAssignment === 'team1' ? 1 : 2;
                                updateRoster(rosterNum, i, 'role', e.target.value);
                              }}
                            >
              ...
              <div className="roster-team">
                ...
                <div className="roster-table-wrapper">
                  <table className="roster-table">
                    ...
                    <tbody>
                      {assignment.teamBRoster.map((player, i) => (
                        <tr key={i}>
                          <td>{i + 1}</td>
                          <td>
                            <input
                              ...
                              value={player.jersey}
                              onChange={(e) => {
                                const rosterNum = coinToss.teamBAssignment === 'team1' ? 1 : 2;
                                updateRoster(rosterNum, i, 'jersey', e.target.value);
                              }}
                              placeholder="#"
                            />
