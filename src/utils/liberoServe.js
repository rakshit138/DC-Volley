 ...
 * Also returns liberoMayServe: true when P1 is a libero and serve is allowed (so UI can show "Libero may serve" popup).
 * @param {Object} gameData - Full game data
 * @param {Object} liberoServeConfig - Libero serve configuration
 * @returns {{ valid: boolean, liberoMayServe?: boolean, message?: string }}
 */
export function validateServeStart(gameData, liberoServeConfig) {
  const currentSet = gameData.currentSet || 1;
  const set = gameData.sets?.[currentSet - 1];
  if (!set) return { valid: true, liberoMayServe: false };
  
  const servingTeam = set.serving;
  if (!liberoServeConfig || !liberoServeConfig[servingTeam]) return { valid: true, liberoMayServe: false };
  
  const config = liberoServeConfig[servingTeam];
  if (!config.enabled || !config.designatedJersey) return { valid: true, liberoMayServe: false };
  
  const lineup = gameData.teams?.[servingTeam]?.lineup || [];
  if (lineup.length === 0) return { valid: true, liberoMayServe: false };
  ...
  if (!p1Player || (p1Player.role !== 'libero1' && p1Player.role !== 'libero2' && p1Player.role !== 'liberocaptain')) {
    return { valid: true, liberoMayServe: false };
  }
  
  // P1 IS a libero. Find who they replaced (handle both number and string position from Firebase)
  const replacements = gameData.liberoReplacements?.[servingTeam] || [];
  const p1Record = replacements.find(r =>
    Number(r.position) === 1 || String(r.position) === '1' || String(r.libero) === p1Jersey
  );
  
  if (!p1Record) {
    const teamName = gameData[`team${servingTeam}Name`] || gameData?.matchInfo?.[`team${servingTeam}Name`] || `Team ${servingTeam}`;
  ...
  // Direct match
  if (originalJersey === designated) return { valid: true, liberoMayServe: true };
  ...
  while (maxDepth-- > 0) {
    if (visited[current]) break;
    visited[current] = true;
    if (current === designated) return { valid: true, liberoMayServe: true };
